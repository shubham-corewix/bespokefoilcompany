...
export default async (request, context) => {
  ...
  // The shared template (/our-kit) is indexed; affiliate copies must never be.
  // Canonical already points at /our-kit, which is exactly right for these.
...
async function lookupFranchisee(slug, context) {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    // Slug convention: FULL NAME, matching the Supabase slug column exactly.
    console.log('No SUPABASE_URL or SUPABASE_ANON_KEY');
    return  null;
  }
...
// In-source declaration - more reliable than netlify.toml on some deploy types
export const config = { path: "/memory-catcher/*" };
